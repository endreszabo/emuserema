...
class PuttyRenderer(Plugin):
    ...
    def render_putty(self):
        for key, service in self.services.items():
            if isinstance(service, SSHservice):
                ...
                print("[HKEY_CURRENT_USER\Software\SimonTatham\PuTTY\Sessions\%s]" % service.tag, file=putty_config)
    ...
    def open_registry(self, world_name):
        if world_name not in self.registries:
            self.registries[world_name]=open("output/SSH/putty/%s.reg" % world_name, 'w')
